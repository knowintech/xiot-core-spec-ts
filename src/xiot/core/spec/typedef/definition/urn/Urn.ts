...
export class Urn extends Extendable {
    ...
    constructor(types: UrnType[], string: string) {
        super();
        ...
        this.init(a[0], a[1], a[2], a[3], a[4]);

        const found = types.find(x => (x === this.type));
        if (found == null) {
            throw new Error('type not matched: ' + string);
        }
    ...
    private init(magic: string, ns: string, type: string, name: string, value: string): void {
        if (magic !== 'urn') {
            throw new Error('type not start with urn: ' + magic);
        }
